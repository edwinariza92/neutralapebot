...
tp_multiplier = 4.0  # Take Profit multiplicador
...
def registrar_operacion(fecha, tipo, precio_entrada, cantidad, tp, sl, resultado=None, pnl=None):
    archivo = 'registro_operaciones_ape.csv'
...
while True:
    try:
        ...
        enviar_telegram(f"🛑 **Bot {symbol} - Estrategia NeutralAPE detenido**\n⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        break
    except Exception as e:
